...
Lock derivation is computed per task to compare declared locks/files with control graph ownership.
The `control_graph.lock_mode` setting controls how it is used:
...
files in that component. Enable with `control_graph.surface_locks.enabled`.

## Scope compliance rollout

Use `control_graph.scope_mode` to control how graph-backed compliance is enforced:

- `off`: skip control graph scope compliance (reports show `status: skipped`).
...
When scope compliance runs with the control graph enabled, owned files resolve to `component:<id>` before
